...
const Search = () => {
    ...
    const [arrangeDesignerList, setArrangeDesignerList] = useState([]);
        ...
        const checkAdmin = async ()=>{
            ...
            setDesignerList(List.map((designer)=>{
                return ({
                    ...
                    cut: designer.cut.toFixed(2),
                    color: designer.color.toFixed(2),
                    purm: designer.purm.toFixed(2),
                })
            }))
            setArrangeDesignerList(List.map((designer)=>{
                return ({
                    id: designer.id,
                    image: `${process.env.REACT_APP_BACKEND_API_URL}/img/${designer.pictureUrl}`,
                    name: designer.name,
    ...
    return (
        <>
            ...
            <Container>
                <DesignerList>
                    {currentFilters["지역"] === "지역전체" ?
                    arrangeDesignerList.sort(function(a,b){
                        if (currentFilters["정렬"] === "인기순"){
                            if (currentFilters["시술"] === "시술전체")
                                return b.star - a.star
                            else if (currentFilters["시술"] === "커트")
                                return b.cut - a.cut
                            else if (currentFilters["시술"] === "염색")
                                return b.color - a.color
                            else if (currentFilters["시술"] === "펌")
                                return b.purm - a.purm
                        }
                        else if (currentFilters["정렬"] === "리뷰순")
                        ...
                        <Link to={`designer/read/${deginer.id}`} style={{ textDecoration: 'none', color: 'inherit'}} key={idx}>
                            <DesignerBox>
                                ...
                                <DesignerIntro>
                                    ...
                                    <InfoText>
                                        {
                                            currentFilters["시술"] === "시술전체" &&
                                            deginer.star
                                        }
                                        {
                                            currentFilters["시술"] === "커트" &&
                                            deginer.cut
                                        }
                                        {
                                            currentFilters["시술"] === "염색" &&
                                            deginer.color
                                        }
                                        {
                                            currentFilters["시술"] === "펌" &&
                    ...
                    )) : arrangeDesignerList.filter(deginer => deginer.location === currentFilters["지역"]).sort(function(a,b){
                        if (currentFilters["정렬"] === "인기순"){
                            if (currentFilters["시술"] === "시술전체")
                                return b.star - a.star
                            else if (currentFilters["시술"] === "커트")
